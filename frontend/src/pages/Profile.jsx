...
import { getMyBlogs, deleteBlog } from "../api/blog.api"; // Removed createBlog import as Form handles it
...
const ConfirmDialog = ({ message, onConfirm, onCancel }) => (
  <div className="pf-overlay" style={{ zIndex: 9999 }}>
    <div className="pf-confirm-card">
      <div className="pf-confirm-icon"><AlertTriangle size={28} /></div>
      <p className="pf-confirm-msg">{message}</p>
      <div className="pf-confirm-actions">
        <button className="pf-btn-ghost" onClick={onCancel}>Cancel</button>
        <button className="pf-btn-danger" onClick={onConfirm}>Delete</button>
      </div>
    </div>
  </div>
);
...
export default function Profile() {
  ...
  const [refreshBlogs, setRefreshBlogs] = useState(0); // Trigger for refreshing list
  ...
  useEffect(() => {
    ...
    loadProfile();
    loadBlogs();
  }, [token, navigate, refreshBlogs]); // Added refreshBlogs dependency
  ...
  // ── handle modal close & refresh ──
  const handleModalClose = () => {
    setShowCreateModal(false);
    // Trigger a refresh of the blog list after closing modal
      ...
      <style>{`
        ...
        .pf-banner::before {
          ...
          background:
            ...
            radial-gradient(circle at 80% 20%, rgba(139,92,246,.3) 0%, transparent 40%);
        }
        .pf-banner-dots {
        ...
        .pf-edit-input {
          width:100%; padding:.65rem .9rem; border-radius:.6rem;
          border:1.5px solid #e2e8f0; background:#fff;
          font-size:.9rem; color:#0f172a; outline:none; box-sizing:border-box;
        ...
        .pf-btn-ghost {
          ...
          padding:.55rem 1.1rem; border-radius:.6rem; font-weight:600; font-size:.875rem; cursor:pointer;
        }
        ...
        .pf-btn-save {
          ...
          color:#fff; box-shadow:0 4px 12px rgba(22,163,74,.3);
        }
        ...
        .pf-btn-icon-edit {
          ...
          background:#eef2ff; color:#6366f1; border:1.5px solid #c7d2fe; cursor:pointer;
          transition: all .15s;
        }
        ...
        .pf-blog-thumb-placeholder {
          width:52px; height:52px; border-radius:.6rem;
          background:linear-gradient(135deg,#e2e8f0,#f1f5f9);
          display:flex; align-items:center; justify-content:center;
          flex-shrink:0; color:#94a3b8; font-size:.75rem; font-weight:500;
          border:1px solid #e2e8f0;
        }
        .pf-blog-title {
        ...
        .pf-blog-excerpt {
          ...
          display:-webkit-box; -webkit-line-clamp:1;
          -webkit-box-orient:vertical; overflow:hidden; max-width:300px;
        }
        .pf-cat-badge {
          display:inline-block; padding:.2rem .6rem; border-radius:999px;
          font-size:.73rem; font-weight:600; background:#eef2ff; color:#6366f1;
        ...
        .pf-modal {
          ...
          animation:pf-modal-in .25s ease;
          position:relative;
        }
        ...
        .pf-confirm-msg { color:#1e293b; font-size:.95rem; font-weight:500; margin-bottom:1.25rem; }
        .pf-confirm-actions { display:flex; justify-content:center; gap:.75rem; }

        @media(max-width:640px){
          .pf-avatar-wrap { padding:0 1.25rem; }
          .pf-user-info { padding:.75rem 1.25rem 1.25rem; }
          .pf-edit-panel { margin:0 1.25rem 1.25rem; }
          .pf-table th, .pf-table td { padding:.75rem 1rem; }
        }
      `}</style>
      ...
      <div className="pf-root">
        <div className="pf-container">
          ...
          <div className="pf-card">
            ...
            <div className="pf-avatar-wrap">
              ...
              <div className="pf-avatar-actions">
                <button
                  className={`pf-btn ${isEditing ? "pf-btn-outline" : "pf-btn-outline"}`}
                  onClick={() => { setIsEditing((v) => !v); setProfileMsg(""); }}
                >
          ...
          <div className="pf-section-header">
            ...
            <button className="pf-btn pf-btn-primary pf-btn-sm" onClick={() => setShowCreateModal(true)}>
          ...
          {blogsLoading ? (
            ...
            <div className="pf-state-box">
              ...
              <button className="pf-btn pf-btn-primary" onClick={() => setShowCreateModal(true)}>
                      ...
                      <tr key={blog._id}>
                        ...
                        <td>
                          <div className="pf-actions-cell">
                            <Link to={`/edit-blog/${blog._id}`} className="pf-btn-icon-edit">
                              <Edit3 size={13} /> Edit
                            </Link>
                            <button className="pf-btn-icon-del" onClick={() => setDeleteTarget(blog._id)}>
      ...
      {showCreateModal && (
        <div className="pf-overlay" onClick={(e) => { if (e.target === e.currentTarget) handleModalClose(); }}>
          <div className="pf-modal">
            ...
            {/* ✅ FIXED: Pass correct props to CreateBlogForm */}
            <CreateBlogForm
